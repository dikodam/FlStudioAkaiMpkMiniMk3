...
attributes = [
    {'name': 'Redo', 'value': 12, 'function': lambda: transport.globalTransport(midi.FPT_Undo, 20)},
    {'name': 'Undo', 'value': 8, 'function': lambda: transport.globalTransport(midi.FPT_UndoUp, 21)},
    {'name': 'Cut', 'value': 15, 'function': lambda: transport.globalTransport(midi.FPT_Cut, 50)},
    {'name': 'Paste', 'value': 11, 'function': lambda: transport.globalTransport(midi.FPT_Paste, 52)},
    {'name': 'Delete', 'value': 14, 'function': lambda: transport.globalTransport(midi.FPT_Delete, 54)},
    {'name': 'Insert', 'value': 13, 'function': lambda: transport.globalTransport(midi.FPT_Insert, 53)},
    {'name': 'Copy', 'value': 10, 'function': lambda: transport.globalTransport(midi.FPT_Copy, 51)},
    {'name': 'Menu1', 'value': 5, 'function': lambda: transport.globalTransport(midi.FPT_Menu, 90)},
    {'name': 'ShowWindow', 'value': 4, 'function': lambda: channels.showCSForm(channels.channelNumber(), 1) if ui.getFocused(1) == 1 else transport.globalTransport(midi.FPT_NextMixerWindow, 1)},
    {'name': 'browserRight', 'value': 3, 'function': lambda: ui.navigateBrowserTabs(midi.FPT_Right)},
    {'name': 'Enter', 'value': 0, 'function': lambda: transport.globalTransport(midi.FPT_Enter, 80)},
    {'name': 'Menu', 'value': 1, 'function': lambda: transport.globalTransport(midi.FPT_ItemMenu, 91)},
    {'name': 'browserLeft', 'value': 2, 'function': lambda: ui.navigateBrowserTabs(midi.FPT_Left)},
    {'name': 'PluginPicker', 'value': 6, 'function': lambda: transport.globalTransport(midi.FPT_F8, 1)},
    {'name': 'Escape', 'value': 7, 'function': lambda: transport.globalTransport(midi.FPT_Escape, 81)},
    {'name': 'SelectAllCH', 'value': 9, 'function': lambda: (channels.selectAll())}
]

def OnInit():
    #Generic.OnInit()
    print('Akai MPK Mini Script by TayseteDj \nYour Device port number is:', device.getPortNumber())
...
def OnMidiMsg(event):
    ...
    if event.midiId == midi.MIDI_PROGRAMCHANGE and (event.midiChan == 10 or event.midiChan == 13):  # FL KNTROL PC
        event.handled = True
    ...
    if event.midiId == midi.MIDI_CONTROLCHANGE and event.midiChan == 10:  ##FL KNTROL CC
        event.handled = True
        if event.data2 > 0:
            if event.data1 == CCKnob_K5:
                ...
                event.handled = True
    if event.midiId == midi.MIDI_CONTROLCHANGE and event.midiChan == 11:  # FL MIXER
        event.handled = True
        ...
        if event.data2 > -0:
            ...
            event.handled = True
    if event.midiId == midi.MIDI_CONTROLCHANGE and event.midiChan == 12:  # FL CREATIVE CC
        event.handled = True
        if event.data2 > 0:
            if event.data1 == 70:
                if event.data2 == 1:
                ...
                if ui.getFocused(1) or ui.getFocused(5):  # Pitch infocus with Channels
                    if event.data2 <= 63:
                        channels.setChannelPitch(channels.channelNumber(), channels.getChannelPitch(channels.channelNumber()) + 0.005)
                        print('set +pitch')
                    elif event.data2 >= 64:
                        channels.setChannelPitch(channels.channelNumber(), channels.getChannelPitch(channels.channelNumber()) - 0.005)
                        print('set -pitch')
                ...
                if ui.getFocused(1) or ui.getFocused(5):  # PAN infocus with Channels
                    if event.data2 <= 63:
                        channels.setChannelPan(channels.channelNumber(), channels.getChannelPan(channels.channelNumber()) + 0.015)
                        print('set +pan')
                    elif event.data2 >= 64:
                        channels.setChannelPan(channels.channelNumber(), channels.getChannelPan(channels.channelNumber()) - 0.015)
                        print('set -pan')
                ...
                event.handled = True
                transport.globalTransport(midi.FPT_F5, 64)
                print('Playlist')
                ...
                if not channels.isChannelSelected(channels.channelNumber()):
                    event.handled = True
                    transport.globalTransport(midi.FPT_NextMixerWindow, 2)
                ...
                if ui.getFocused(1):
                    ...
                    playlist.triggerLiveClip(track, block, midi.TLC_Fill)
                ...
                if 0 <= event.data2 <= 110:
                    ...
                    print('ESCAPE ALL')
                    transport.globalTransport(midi.FPT_F12, 71)
                ...
                transport.globalTransport(midi.FPT_F4, 100)
                time.sleep(0.01)
                transport.globalTransport(midi.FPT_Escape, 81)

            elif event.data1 == 31:
                transport.globalTransport(midi.FPT_Copy, 51)
                print('Copy Pattern')

            elif event.data1 == 27:
                transport.globalTransport(midi.FPT_Paste, 52)
                print('Paste Pattern')
                ...
                if ui.getFocused(1):  # SetMixerRack infocus with Channels
                    if 1 <= event.data2 <= 63 and 0 < channels.getTargetFxTrack(channels.channelNumber()) < 125:
                        ...
                        m = channels.channelNumber()
                        channels.processRECEvent(channels.getRecEventId(m) + REC_Chan_FXTrack, channels.getTargetFxTrack(m) + 1, REC_Control | REC_UpdateControl)
    ...
    if event.midiId == midi.MIDI_PROGRAMCHANGE and event.midiChan == 12:  # FL CREATIVE PC
        ...
        if event.data2 < 1:
            if not channels.isChannelSelected(channels.channelNumber()):  # While Focusing Mixer
                if event.data1 == 4:
                    ...
                    transport.globalTransport(midi.FPT_NextMixerWindow, 1)
                    channels.deselectAll()
                    print('MixerWindow1')
                elif event.data1 == 5:
                ...
                if event.data1 == 4:
                    ...
                    transport.globalTransport(midi.FPT_Cut, 50)
                    print('Cut')
                elif event.data1 == 7:
                    transport.globalTransport(midi.FPT_Paste, 52)
                    print('Paste')
                elif event.data1 == 0:
                    transport.globalTransport(midi.FPT_Delete, 54)
                    print('Delete')
                elif event.data1 == 1:
                    transport.globalTransport(midi.FPT_Insert, 53)
                    print('Insert')
                elif event.data1 == 2:
                    transport.globalTransport(midi.FPT_Copy, 51)
                    print('Copy')
                elif event.data1 == 3:
                    transport.globalTransport(midi.FPT_SaveNew, 93)
                    print('SaveAS')

            elif not ui.getFocused(0) and not ui.getFocused(1) and not ui.getFocused(3) and not ui.getFocused(5):  # While Focusing Anything but not Mixer, pianoRoll, plugins, or Channels
                if event.data1 == 4:
                    print('Pat/Song')
                    transport.setLoopMode()
                elif event.data1 == 5:
...
def OnControlChange (event) :
    if event.midiId == (midi.MIDI_CONTROLCHANGE) and (event.midiChan == 13):## FL PLUG
            ...
            if 70 <= event.data1 <= 77: ## Controls and FX 1
                if plugins.isValid(channels.channelNumber()) == 0 and not ui.getFocused(6): ## samplers
                    ...
                    if event.data1 == 70:
                        ...
                        m = channels.channelNumber()
                        eventId = midi.REC_Chan_FCut + channels.getRecEventId(channels.channelNumber())
                        ...
                        step = 1 if event.data2 >= 64 else -1
                        m = channels.channelNumber()
                        eventId = midi.REC_Chan_SwingMix + channels.getRecEventId(channels.channelNumber())
                        newValue = channels.incEventValue(eventId, step)
                        general.processRECEvent(eventId, newValue, midi.REC_UpdateValue | midi.REC_UpdateControl)
                    elif event.data1 == 71:
                        step = 1 if event.data2 >= 64 else -1
                        m = channels.channelNumber()
                        eventId = midi.REC_Chan_FRes + channels.getRecEventId(channels.channelNumber())
                        newValue = channels.incEventValue(eventId, step)
                        general.processRECEvent(eventId, newValue, midi.REC_UpdateValue | midi.REC_UpdateControl)
                    elif event.data1 == 73:
                        step = 1 if event.data2 >= 64 else -1
                        m = channels.channelNumber()
                        eventId = midi.REC_Chan_OfsFCut + channels.getRecEventId(channels.channelNumber())
                        newValue = channels.incEventValue(eventId, step)
                        general.processRECEvent(eventId, newValue, midi.REC_UpdateValue | midi.REC_UpdateControl)
                    elif event.data1 == 74:
                        step = 1 if event.data2 >= 64 else -1
                        m = channels.channelNumber()
                        eventId = midi.REC_Chan_OfsFRes + channels.getRecEventId(channels.channelNumber())
                        newValue = channels.incEventValue(eventId, step)
                        general.processRECEvent(eventId, newValue, midi.REC_UpdateValue | midi.REC_UpdateControl)
                ...
                ##                    print(apply)
                    ...
                    for i in range(70, 77):
                        ...
                        if ui.getFocused(5):
                            ...
                            event.handled = True
    if event.midiId == (midi.MIDI_CONTROLCHANGE) and (event.midiChan == 9): ##FL FPC CC
        event.handled = True
...
'''    if (event.midiId == midi.MIDI_CONTROLCHANGE) and (event.midiChan == 13):
        event.handled = False
...
#   instead of a Note off midi message
    if (event.status == midi.MIDI_NOTEON) and (event.data2 == 0) and (event.midiChan == 13):
...
''''''ui.setHintMsg
                channels.processRECEvent(midi.REC_Chan_FCut + channels.getRecEventId(channels.channelNumber()), ((-1) + midi.REC_GetValue), midi.REC_Control | REC_UpdateValue)
                channels.processRECEvent(channels.getRecEventId(m) + REC_Chan_OfsFCut, (channels.getRecEventId(m) + midi.REC_GetValue), REC_Control | REC_UpdateValue)
                device.midiOutMsg(176, 12, 18, 120)
                device.midiOutMsg(midi.MIDI_CONTROLCHANGE, 12, 22, 81)
                ...
                if event.data2 <= 63:
                    event.handled = True
                    print('fcut')
                    channels.processRECEvent(midi.REC_Chan_FCut + channels.getRecEventId(channels.channelNumber()), channels.processRECEvent(midi.REC_Chan_FCut + channels.getRecEventId(channels.channelNumber()), 1, midi.REC_GetValue) + 1 , midi.REC_UpdateValue)
                elif event.data2 >= 64:
                    event.handled = True
                    print('fcut')
                    channels.processRECEvent(midi.REC_Chan_FCut + channels.getRecEventId(channels.channelNumber()), channels.processRECEvent(midi.REC_Chan_FCut + channels.getRecEventId(channels.channelNumber()), 1, midi.REC_GetValue) - 1 , midi.REC_UpdateValue)
            ...
            if event.data1 == 74: 
                print('TEST')    
                channels.processRECEvent(midi.REC_Chan_FCut + channels.getRecEventId(channels.channelNumber()), channels.processRECEvent(midi.REC_Chan_FCut + channels.getRecEventId(channels.channelNumber()), 0, midi.REC_GetValue) + 0.001 , midi.REC_UpdateValue)
if event.data1 == 74:
                if event.data2 <= 1:
                    ##channels.processRECEvent(midi.REC_Chan_FCut + channels.getRecEventId(channels.channelNumber()), (+ midi.FromMIDI_Max), midi.REC_Control | midi.REC_UpdateControl(0.05) | midi.REC_FromMIDI)
                    general.processRECEvent(midi.REC_Chan_FCut + channels.getRecEventId(channels.channelNumber()), (- (64 - int(event.data2))) if (event.data2 < 65) else (int(event.data2) - 63), midi.REC_UpdateControl | midi.REC_UpdateValue | midi.REC_ShowHint)
                    print('set +cutoff')
            ...
            if event.data1 == 75:
                if event.data2 <= 1:
                    channels.processRECEvent(midi.REC_Chan_FCut + channels.getRecEventId(channels.channelNumber()), round((event.controlVal + 0.001) + midi.FromMIDI_Max), midi.REC_Control | midi.REC_UpdateControl | midi.REC_FromMIDI)
                    print('set +cutoff')
                elif event.data2 >= 64:
                    channels.processRECEvent(midi.REC_Chan_FCut + channels.getRecEventId(channels.channelNumber()), round((event.controlVal - 0.001) - midi.FromMIDI_Max), midi.REC_Control | midi.REC_UpdateControl | midi.REC_FromMIDI)
                    print('set -cutoff')
